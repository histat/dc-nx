...
{
static const char *drum_cache = "\\Temp\\drum.pcm";
int d;
FILE *fp;
...
int i;
	...
	init_pitch();
	if (load_wavetable("\\Temp\\wavetable.dat")) return 1;
...
{
	audio_submit_block((uint8_t *)final_buffer[current_buffer].samples, buffer_samples*4);
...
void OrgBufferFinished(void *crap)		//int channel, int buffer_no)
{
	stat("OrgBufferFinished()");
	buffers_full = false;
}
...
{
	// the original sampling rate of the samples in the wavetable
	#define	SAMPLE_K	 	 11025

	// compute how quickly or slowly, to play back the wavetable sample
	int new_sample_rate = GetNoteSampleRate(note, pitch);
	chan->isample_inc = (new_sample_rate << PHASE_CSF) / SAMPLE_K;
	
	chan->wave = wave;
	chan->iphaseacc = 0;
	
	//stat("note_open: new note opened for channel %08x at sample_inc %.2f, using wave %d\n", chan, chan->sample_inc, chan->wave);
}
...
uint8_t pos1;
	...
	{
		pos1 = (chan->iphaseacc >> PHASE_CSF);
		
		audioval = wavetable[wave].samples[pos1];
		...
		chan->iphaseacc += chan->isample_inc;
		if (chan->iphaseacc >= 256<<PHASE_CSF)
			chan->iphaseacc -= 256<<PHASE_CSF;
	}
}
...
int out_position;
	...
	stat("generate_music: cb=%d beats=%d", current_buffer, buffer_beats);
