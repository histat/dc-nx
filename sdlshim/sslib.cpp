...
bool SSInit(void)
{
	...
	mixbuffer = (uint8_t *)malloc(AUDIO_SAMPLES * AUDIO_CHANNELS * 2);
...
int SSEnqueueChunk(int c, signed short *buffer, int len, void *userdata, \
				void(*FinishedCB)(int, void *))
{
SSChannel *chan;
SSChunk *chunk;
	...
	if (++chan->tail >= MAX_QUEUED_CHUNKS)
		chan->tail = 0;
...
int SSPlayChunk(int c, signed short *buffer, int len, void *userdata, void(*FinishedCB)(int, void *))
{
...
// if not, does nothing. it does NOT call this finished-callback.
...
void SSAbortChannelByUserData(void *ud)
{
int c;
	SSLockAudio();
...
static int AddBuffer(SSChannel *chan, int bytes)
{
	...
	//stat("%d: Channel %x: Copying %d bytes from chunk @ %08x -- pos=%d, len=%d", \
		//SDL_GetTicks(), chan, bytes, chunk->bytebuffer, chunk->bytepos, chunk->bytelength);
